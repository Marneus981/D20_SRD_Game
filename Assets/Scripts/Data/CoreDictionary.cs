...
public class CoreDictionary<TKey, TValue> : ISerializationCallbackReceiver, 
                                            ...
                                            ISerializable
/*
As with CoreSet: Two implementations:
    ISerializationCallbackReceiver (to serialize data), The Rest (to conform)
        (For Completion, Dictionary Declaration) 
...
*/
{
    [SerializeField] List<TKey> keys = new List<TKey>(); //Serialized by Unity
    [SerializeField] List<TValue> values = new List<TValue>(); //Serialized by Unity
